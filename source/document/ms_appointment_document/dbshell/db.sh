...
psql -v ON_ERROR_STOP=1 --username "${DB_USER}" --dbname "${DB_NAME}" <<-EOSQL
  ...
  COMMENT ON TABLE  ${DB_TABLENAME}                      IS 'Employee appointment document';
  ...
  COMMENT ON COLUMN ${DB_TABLENAME}.id_employee          IS 'Employee appointment Id';
  COMMENT ON COLUMN ${DB_TABLENAME}.id_organization      IS 'Organization of appointment';
  COMMENT ON COLUMN ${DB_TABLENAME}.id_department        IS 'Appointment department';
  COMMENT ON COLUMN ${DB_TABLENAME}.id_job_title         IS 'Department employee position';
  COMMENT ON COLUMN ${DB_TABLENAME}.justification        IS 'Justification for appointment';
