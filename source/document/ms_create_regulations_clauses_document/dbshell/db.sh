...
psql -v ON_ERROR_STOP=1 --username "${DB_USER}" --dbname "${DB_NAME}" <<-EOSQL
  CREATE TABLE public.${DB_TABLENAME} (
    ...
    id_user                bigint,
    prefix                 smallint,
	id_regulations_clauses bigint,
    id_transaction         text,
    UNIQUE (id, id_basis)
	);

  COMMENT ON TABLE  ${DB_TABLENAME}                        IS 'Create regulations clauses document';
  ...
  COMMENT ON COLUMN ${DB_TABLENAME}.name                   IS 'Regulations_clauses name';
  ...
  COMMENT ON COLUMN ${DB_TABLENAME}.id_regulations_clauses IS 'Regulations_clauses ID';
